...
#### For a final visualization, the graph below shows the changes in flooring times for 40x12 units. These are the units that have been most commonly produced during the data collection process, and therefore have undergone the most process changes. 


```{r echo=FALSE, message=FALSE, warning=FALSE}
p2 <- ggplot(flooring, aes(x=c("40x12"), y=Total_Flooring_Time, fill=Type )) + 
  geom_col(na.rm = TRUE) +
  scale_fill_hue(c = 40) +
  theme() +
